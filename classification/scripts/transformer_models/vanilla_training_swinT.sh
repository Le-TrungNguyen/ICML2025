...
for model in "${models[@]}"; do
  ...
  common_args="$common_config_args $common_data_args $common_model_args $common_trainer_args $common_seed_args"

  for dataset in "${!datasets[@]}"; do
    num_classes=${datasets[$dataset]}
    echo "  Processing dataset: $dataset with num_classes: $num_classes"

    specific_logger_args="--logger.save_dir ${saved_location}/setup$setup/$model/$dataset/vanilla"
    ...
    for num_of_finetune in "${num_of_finetune_list[@]}"; do
      ...
      python trainer_cls_linear.py ${all_args} --logger.exp_name vanilla_l${num_of_finetune} --model.num_of_finetune $num_of_finetune
    done
  done
done
